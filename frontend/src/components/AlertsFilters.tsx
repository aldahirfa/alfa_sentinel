...
const VIEW_OPTIONS: { value: "activas" | "todos"; label: string; icon: string }[] = [
  { value: "activas", label: "Activas", icon: "ph ph-pulse" },
...
export default function AlertsFilters({
  ...
  const hasFilters = Boolean(search || severity || status || since || rule || view !== "activas");
    ...
    <section className="soc-panel rounded-2xl p-4 flex flex-col gap-4">
      <div className="flex flex-col lg:flex-row lg:items-center gap-3">
        <div className="relative flex-1">
          <i
            className="ph ph-magnifying-glass absolute left-3.5 top-1/2 -translate-y-1/2"
          ...
          <input
            ...
            placeholder="Buscar por alerta, endpoint o descripción..."
            className="w-full pl-10 pr-4 py-2.5 rounded-xl text-[12px] outline-none transition-premium"
            style={{
              background: "var(--surf2)",
              border: "1px solid var(--line-soft)",
              color: "var(--tx)",
              boxShadow: search ? "0 0 0 2px var(--brand-soft)" : "none",
            }}
          />
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2.5 pt-3 border-t" style={{ borderColor: "var(--line-soft)" }}>
        <div className="flex items-center gap-1.5 mr-1">
          ...
          <span className="text-[9px] font-bold tracking-[.13em] uppercase" style={{ color: "var(--tx-mute)" }}>
            Filtros
          </span>
        </div>
          ...
          {SEVERITY_OPTIONS.map((opt) => {
            ...
            return (
              <button
                ...
                style={active
                  ? {
                      background: opt.tone ? `color-mix(in srgb, ${opt.tone} 14%, var(--surf3))` : "var(--brand-soft)",
                      color: opt.tone || "var(--brand)",
                    }
                  : { background: "transparent", color: "var(--tx-mute)" }}
              >
                {opt.label}
        ...
        {hasFilters && (
          <button
            onClick={reset}
            className="ml-auto flex items-center gap-1.5 px-3 py-2 rounded-xl text-[10px] font-semibold cursor-pointer border transition-premium btn-hover"
            style={{ background: "transparent", borderColor: "var(--line-soft)", color: "var(--tx-mute)" }}
          >
