...
export default function IncidentesSummaryCards({ summary }: Props) {
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      <Card icon="ph-fill ph-siren" label="Incidentes críticos" value={summary.critical_incidents} color="var(--crit)" />
      <Card icon="ph ph-warning" label="Alertas activas" value={summary.active_alerts} color="var(--info)" />
      <Card icon="ph ph-plugs" label="Hosts aislados" value={summary.isolated_hosts} />
      <Card
        icon="ph ph-timer"
        label="MTTR promedio"
        value={summary.mttr_minutes !== null ? `${summary.mttr_minutes} min` : "—"}
      />
    </div>
  );
}
